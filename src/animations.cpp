...
void Animations::Static::rainbow(std::function<void(uint32_t, uint8_t, uint8_t, uint8_t)> setPixelColor, uint32_t ledCount){
    for (uint32_t i = 0; i < ledCount; i++){
      setPixelColor(i, Helper::wheel(i, ledCount), Helper::wheel((i+ledCount/3)%ledCount, ledCount), Helper::wheel((i+ledCount*2/3)%ledCount, ledCount));
    }
}
...
void Animations::Static::rainbow(uint8_t* memory, uint32_t ledCount){
    for (uint32_t i = 0; i < ledCount; i++){
      ...
      memory[i*3+2] = Helper::wheel((i+ledCount*2/3)%ledCount, ledCount);
    }
}

void Animations::Dynamic::rain(uint8_t* memory, 
                               uint32_t ledCount, 
                               uint8_t dropRadius,
                               uint32_t dropCountDivisor,
                               uint8_t reduction 
                               ){
    // evaporate
...
void Animations::Dynamic::rainbow(std::function<void(uint32_t, uint8_t, uint8_t, uint8_t)> setPixelColor, uint32_t ledCount, uint32_t progress){
