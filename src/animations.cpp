...
void Animations::Dynamic::knightrider(
    std::function<void(uint32_t, uint8_t, uint8_t, uint8_t)> setPixelColor,
    uint32_t ledCount, uint32_t &progress, uint8_t width)
{
    //the least significant bit of progress determines the direction
    //the rest determines the position
    uint32_t position = progress >> 1;
    bool direction = progress & 1;
    uint8_t red = 0;
    for (uint32_t i = 0; i < ledCount; i++)
    {
